	...
	this.listPromocao = function() {
        ...
        tx.executeSql(sql, [], function (tx, results) {
            ...
            for (i = 0; i < len; i++){  
                ...
                flista.append('<li><a  data-pid="'+results.rows.item(i).id+'" href="javascript:promocao.getPromocao(\''+results.rows.item(i).id+'\')" class="mpopup">'+results.rows.item(i).nome+'<p>Ref:'+results.rows.item(i).ref+' Válido até '+results.rows.item(i).expira+'</p></a></li>');
